...
/**
 * Category → Sub Category → Service/Product → Deal discovery page — the core of the customer
 * catalogue (marketplace architecture plan, Phase 6). Reuses `category.css` and the
 * `Tabs`/`ChipSet`/`SkyProductCardWC` components exactly as the existing static `/category/:slug`
 * page does, but reads real Vendor/Branch/Deal data from `GET /catalog/*` — only active,
 * approved deals with an active vendor/branch/catalog item are ever returned (enforced
 * server-side in `catalog.service.ts`).
 *
...
export function MarketplaceCategory() {
  ...
  const requireAuthOrRedirect = () => {
    ...
    navigate(`/sign-in?next=${encodeURIComponent(`/marketplace/${slug}`)}`);
    return false;
  };
  ...
  if (categoryError || !category) {
    return (
      <div className="category-page category-page--empty">
        ...
        <sky-info-card icon="search_off" heading="Category not found" subheading={categoryError || 'Try browsing the marketplace.'} />
  ...
  return (
    <div className="category-page">
      <title>{`${category.name} | Marketplace | MSD`}</title>
      ...
      <Breadcrumb
        ...
        items={[{ label: 'Home', to: '/' }, { label: 'Marketplace', to: '/marketplace' }, { label: category.name }]}
      />
