...
/**
 * Real, backend-driven product cart — `Cart`/`CartItem` from Phase 7 (single vendor+branch per
 * cart, product deals only). Reuses `cart.css`/`cart-item__*`/`sky-info-card` exactly like the
 * existing mock `/cart` page; kept as its own `/marketplace/cart` route so the still-working
 * mock storefront's `localStorage` cart is untouched (same reasoning as the Phase 6 catalogue
 * living under `/marketplace` instead of rewiring `/category`/`/products`).
 */
export function MarketplaceCart() {
  ...
  const doCheckout = () => {
    ...
    // see the Phase 9 architecture plan), so a page refresh mid-payment retries against the
    // same Order instead of silently creating another one here.
    navigate('/checkout');
  };
  ...
  return (
    <div className="cart-page">
      <title>Cart | Marketplace | MSD</title>
      ...
      <div className="cart-page__inner">
        ...
        {items.length === 0 ? (
          <div className="cart-page__empty">
            <sky-info-card icon="shopping_bag" heading="Your cart is empty" subheading="Browse the marketplace to add products." />
            ...
            <aside className="cart-page__summary" aria-label="Order summary">
              <sky-card variant="outlined" className="cart-summary-card">
                <div className="cart-summary">
                  ...
                  <OutlinedButton className="cart-summary__continue-btn" onClick={() => navigate('/marketplace')}>
                    Continue browsing
                  </OutlinedButton>
                </div>
              </sky-card>
        ...
        <Link to="/marketplace/bookings" className="field-hint">View your bookings →</Link>{' '}
